    ...
    <div className="bg-white rounded-2xl shadow-sm border border-zinc-200 p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <div className="bg-zinc-50 border border-zinc-100 rounded-xl p-3">
          ...
          <p className="text-base font-bold font-mono text-zinc-900 mt-0.5">{formatMoney(data.cashSales)}</p>
        </div>
        <div className="bg-zinc-50 border border-zinc-100 rounded-xl p-3">
          <span className="text-[11px] text-zinc-500 font-bold block">Ventas Tarjeta</span>
          <p className="text-base font-bold font-mono text-zinc-900 mt-0.5">{formatMoney(data.cardSales)}</p>
        </div>
        <div className="bg-red-50/50 border border-red-100 rounded-xl p-3">
          <span className="text-[11px] text-red-600 font-bold block">Devoluciones</span>
          <p className="text-base font-bold font-mono text-red-700 mt-0.5">-{formatMoney(data.cashRefunds)}</p>
        </div>
        <div className="bg-amber-50/60 border border-amber-200/60 rounded-xl p-3">
          ...
          <p className="text-base font-bold font-mono text-amber-900 mt-0.5">-{formatMoney(data.totalExpenses || 0)}</p>
        </div>
      </div>
      ...
      <div className="border-t border-zinc-100 pt-4 flex justify-between items-end">
        ...
        <span className="text-3xl font-black font-mono text-zinc-900 tracking-tight">{formatMoney(data.expectedBalance)}</span>
      </div>
    </div>
  );
}